 ...
 * Teff=4250K, log(g)=2.0, [Fe/H]=0.0, xi=1.0 km/s, l=1.0H_p, M=1M_Sun, R=6.4761D+10cm
 *
 * @author Ian
 */
public class ScaleT4250g20{

    public static double phxRefTeff = 4250.0;
    public static double phxRefLogEg = Math.log(10.0) * 2.0;  //base e!
//He abundance from  Grevesse Asplund et al 2010
 ...
 1.00000000000000000e+02
    ...
    public static double[] logPhxRefTau64() {
        ...
        int numPhxDep = ScaleT4250g20.phxRefTau64.length;
        double logPhxRefTau64[] = new double[numPhxDep];
        for (int i = 1; i < numPhxDep; i++) {
            logPhxRefTau64[i] = Math.log(ScaleT4250g20.phxRefTau64[i]);
        }
 ...
 8.42235419676150195e+03
        ...
        for (int i = 0; i < numDeps; i++) {
            phxRefTemp[i] = ToolBox.interpol(ScaleT4250g20.logPhxRefTau64(), phxRefTemp64, tauRos[1][i]);
            scaleTemp[0][i] = teff * phxRefTemp[i] / ScaleT4250g20.phxRefTeff;
...
//exponents in scaling with g:
        double gexpTop = 0.54; //top of model
        double gexpBottom = 0.64; //bottom of model
...
// factor for scaling with A_He:
        ...
        for (int i = 0; i < numDeps; i++) {
            ...
            //}
            logPhxRefPGas[i] = ToolBox.interpol(ScaleT4250g20.logPhxRefTau64(), logPhxRefPGas64, tauRos[1][i]);
            //if (i%10 == 0){
            ...
            //}
            scalePGas[1][i] = thisGexp*logEg + logPhxRefPGas[i] - thisGexp*ScaleT4250g20.phxRefLogEg;
            //if (i%10 == 0){
...
//exponents in scaling with g:
        double gexpTop = 0.48; //top of model
        double gexpBottom = 0.33; //bottom of model
        double gexpRange = (gexpBottom - gexpTop);
        double tauLogRange =  tauRos[1][numDeps-1] -  tauRos[1][0]; 
        double thisGexp;
...
// factor for scaling with A_He:
        ...
        for (int i = 0; i < numDeps; i++) {
            ...
            //System.out.println("i " + i);
            //}
            logPhxRefPe[i] = ToolBox.interpol(ScaleT4250g20.logPhxRefTau64(), logPhxRefPe64, tauRos[1][i]);
            thisGexp = gexpTop + gexpRange * (tauRos[1][i] - tauRos[1][0]) / tauLogRange;
            if (tauRos[0][i] < 0.1){
            ...
            //}
            //scaling with g 
            scalePe[1][i] = thisGexp*logEg + logPhxRefPe[i] - thisGexp*ScaleT4250g20.phxRefLogEg;
            //if (i%10 == 0){
            //System.out.println("After g scaling: pe " + logE*scalePe[1][i]);
            //}
            //scale with Teff:
            scalePe[1][i] = thisOmega*teff + scalePe[1][i] - thisOmega*ScaleT4250g20.phxRefTeff;
            //if (i%10 == 0){
